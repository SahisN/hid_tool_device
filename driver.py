...
HID_KEYCODES = {
    ...
    'A': 4 + 0x80, 'B': 5 + 0x80, 'C': 6 + 0x80, 'D': 7 + 0x80, 'E': 8 + 0x80,
...
def type_character(char):
    if char in HID_KEYCODES:
        # Press the key corresponding to the character
        write_report(NULL_CHAR*2 + chr(HID_KEYCODES[char]) + NULL_CHAR*5)
        # Release the key
...
def type_string(word):
    with open('/dev/hidg0', 'rb+') as fd:
        for char in word:
            if char in HID_KEYCODES:
                # Press the key
                fd.write((NULL_CHAR*2 + chr(HID_KEYCODES[char]) + NULL_CHAR*5).encode())
                # Release the key
